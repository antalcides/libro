...
every state/.style = {draw=blue!50,very thick,fill=blue!20,on chain=1},
    ...
    \fill[ptctitle] plot[smooth,domain=0:\angle] (pi/180*\x,{cos(\x)}) |- (0,0);
    ...
    \draw[titlebgdark] plot[smooth,domain=0:360] (pi/180*\x,{cos(\x)});
    \draw[titlebgdark] (-2,0) circle (1);
    \draw[gray,thick] ({-2+cos(\angle)},0) --({-2+cos(\angle)},{sin(\angle)})circle(0.5pt) ;
    \draw[ocre,thick,fill=ocre] (pi/180*\angle,0)circle(0.5pt) --(pi/180*\angle,{cos(\angle)})circle(0.5pt) ;
    \draw[ocre,thick,fill=ocre] (-2,0)circle(0.5pt) --({-2+cos(\angle)},0)circle(0.5pt) ;
    
%    % trasladando el punto del circulo a la funci\'on
%    \draw[fill=zanahoria,zanahoria,dashed] (-2,0) +(\angle:1) circle (1pt) -- (pi/180*\angle,{cos(\angle)}) circle (1pt);
    %%%%%%%%%
    \coordinate (D) at (0, 1.45);
    %\draw(2,0.75)\node (D) [state] {};
    \node (d1) [X,fill=blue!30,right=11mm of D]     {Funci\'on Coseno};
    \node (d2) [X,fill=green!30,right=of d1.east]         {$\cos({\pgfmathparse{\angle}\pgfmathresult}^\circ)=\pgfmathparse{cos(\angle)}\pgfmathresult$};
      \end{tikzpicture}
}
\end{document}
%orden en consola
